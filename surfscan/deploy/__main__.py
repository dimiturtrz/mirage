"""Deploy cost-model entry point — profile model footprints, project onto accelerators.
    ...
    python -m surfscan.deploy accelerators  # cited edge-accelerator spec table -> docs/ACCELERATORS.json
    python -m surfscan.deploy bank          # PatchCore bank-memory model -> docs/BANK_COST.json
    python -m surfscan.deploy project       # join model x accelerator -> docs/DEPLOY_PROJECTION.json
