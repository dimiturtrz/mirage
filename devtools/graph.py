"""Import-graph architecture diagnostic + fitness gate (bag/169). grimp builds the honest import graph;
networkx ranks structure and `--assert` turns the measurable properties into a RATCHETED gate (the metric
arch axis import-linter's categorical contracts can't express):
...
# graph so the blocking rules start CLEAN and ratchet: fan-in&out both>8, file>750, cycles. The line
# FLOOR is OFF (file_min=0) — owner decision: no honest floor exists, the small files are SSOT/strategy/
# shared-vocab leaves; "too thin" is a responsibility judgment, not a line count.
...
def _undersized(files: list[tuple[str, int]], mn: int) -> list[str]:
    if mn <= 0:                                     # floor OFF (owner: no honest line floor — SSOT/strategy leaves)
        return []
...
def _run_assert(packages: list[str]) -> int:
    ...
    if advisory:
        over = len(advisory) - _ADVISORY_PREVIEW
        extra = f"\n  … +{over} more" if over > 0 else ""
        body = "\n  ".join(advisory[:_ADVISORY_PREVIEW]) + extra
...
def main():
    ...
    ap.add_argument("--packages", nargs="+", default=["core", "surfscan"], help="root packages to graph")
