...
Emits deployment/models_params.json: the measured `components` (atoms), the `detectors` (shippable
...
class Profiler:
    ...
    def document(size: int, dev: str) -> dict:
        """The deployment/models_params.json payload: measured components + detector op-classes + bank."""
        components = Profiler.measure(size, dev)
        return {
            "note": "generated by `python -m surfscan.deploy profile` — measured single-frame footprint",
            ...
            "detectors": [{"name": d.name, "op_class": d.op_class, "pieces": list(d.pieces),
                           "has_bank": d.has_bank} for d in Profiler._DETECTORS],
