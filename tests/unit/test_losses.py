...
def test_masked_recon_perfect_is_zero():
    ...
    assert Losses.masked_recon_loss(x.clone(), x, valid).item() < 1e-6
...
def test_masked_recon_ignores_background():
    ...
    assert Losses.masked_recon_loss(recon, x, valid).item() < 1e-6
...
def test_masked_recon_known_value():
    ...
    assert abs(Losses.masked_recon_loss(recon, x, valid).item() - 1.0) < 1e-4   # mean SE = 1


def test_kl_standard_normal_is_zero():
    mu = torch.zeros(4, 16)
    logvar = torch.zeros(4, 16)                    # var = 1 -> N(0,1) -> KL 0
    assert abs(Losses.kl_loss(mu, logvar).item()) < 1e-6


def test_kl_positive_off_standard():
    assert Losses.kl_loss(torch.ones(4, 16), torch.zeros(4, 16)).item() > 0    # shifted mean -> KL > 0
