...
ModelCard._render(run_name, row) builds the markdown from a metrics/params row dict (no mlflow, no file write).
Equivalence classes: a full row (headline + per-cat + params) vs an empty row (the no-metrics fallback).
"""
from surfscan.evaluation.modelcard import ModelCard
...
def test_render_full_row():
    ...
    md = ModelCard._render("patchcore_rgb_greedy", row)
