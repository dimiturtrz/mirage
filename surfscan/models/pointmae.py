...
from surfscan.models.pointmae_group import farthest_point_sample, index_points, square_distance
...
def _install_op_shims():
    ...
    if "pointnet2_ops" not in sys.modules:
        ...
        pu.furthest_point_sample = lambda xyz, n: farthest_point_sample(xyz, n).int()   # (B,n) idx
        pu.gather_operation = lambda feats, idx: (                                       # (B,C,N),(B,S)->(B,C,S)
            index_points(feats.transpose(1, 2).contiguous(), idx.long()).transpose(1, 2).contiguous())
