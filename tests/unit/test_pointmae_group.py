...
def test_square_distance_matches_cdist():
    ...
    got = PointmaeGroup.square_distance(a, b)
...
def test_index_points_gathers_batchwise():
    ...
    out = PointmaeGroup.index_points(pts, torch.tensor([[0, 2], [1, 3]]))
...
def test_fps_spans_two_clusters():
    ...
    picked = PointmaeGroup.index_points(xyz, PointmaeGroup.farthest_point_sample(xyz, 4))[0]
