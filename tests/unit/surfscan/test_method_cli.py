...
def test_build_method_wires_config_and_device():
    ...
    run_name, method = MethodCli.build_method(FakeMethod, FakeCfg, args, "cpu")
...
def test_build_method_run_name_optional():
    ...
    run_name, _ = MethodCli.build_method(NoNameMethod, FakeCfg, args, "cpu")
    assert run_name is None


def test_method_spec_builds_spec_and_args():
    spec = MethodCli.method_spec("fake", FakeMethod, FakeCfg)
