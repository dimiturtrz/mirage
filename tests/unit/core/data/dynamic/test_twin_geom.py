...
class TestParseObj:
    def test_verts_and_faces_zero_indexed(self, tmp_path):
        ...
        verts, faces = TwinGeom.parse_obj(str(obj))
    ...
    def test_face_slash_takes_vertex_index_only(self, tmp_path):
        ...
        _, faces = TwinGeom.parse_obj(str(obj))
...
class TestBackproject:
    def test_center_is_origin_edges_scale(self):
        ...
        xyz = TwinGeom.backproject(depth, fx=2.0, fy=2.0)
    ...
    def test_infinite_depth_maps_to_zero(self):
        ...
        xyz = TwinGeom.backproject(depth, fx=1.0, fy=1.0)
...
class TestDeform:
    def test_only_z_changes_bounded_by_amp(self):
        ...
        out, amp = TwinGeom.deform(verts, rng, sign=1.0)
    ...
    def test_sign_flips_push_direction(self):
        ...
        verts[:, 2] = 0.0
        dent, _ = TwinGeom.deform(verts, np.random.RandomState(2), sign=1.0)
        bump, _ = TwinGeom.deform(verts, np.random.RandomState(2), sign=-1.0)   # same rng -> same patch
