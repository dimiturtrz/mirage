...
def _enc_block(ci, co):
    return nn.Sequential(nn.Conv2d(ci, co, 4, 2, 1), nn.BatchNorm2d(co), nn.SiLU())


def _dec_block(ci, co, last=False):
...
class ConvVAE(nn.Module):
    def __init__(self, in_ch=3, base=32, latent=256, size=256, depth=5):
        super().__init__()
        self.in_ch, self.size, self.depth = in_ch, size, depth
        chs = [in_ch] + [base * 2 ** i for i in range(depth)]   # e.g. 3,32,64,128,256,512
        self.enc = nn.ModuleList(_enc_block(chs[i], chs[i + 1]) for i in range(depth))
        ...
        self.dec = nn.ModuleList(
            _dec_block(dchs[i], dchs[i + 1], last=(i == depth - 1)) for i in range(depth))
    ...
    def decode(self, z):
        x = self.fc_dec(z).view(-1, self.bottleneck_ch, self.feat, self.feat)
        for b in self.dec:
            x = b(x)
        return x
