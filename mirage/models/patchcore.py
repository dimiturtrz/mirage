...
class PatchCore:
    ...
    @torch.no_grad()
    def fit(self, x, batch=16, coreset=0.1, seed=0):
        feats = []
        ...
        if coreset < 1.0:
            ...
            idx = torch.randperm(len(feats), generator=g, device=feats.device)[:int(len(feats) * coreset)]
            feats = feats[idx]                                      # random coreset (greedy is the refinement)
        self.bank = feats
        return self
