...
4. **Stage 3 — Edge deploy (hardware-gated road).** ONNX → an **edge accelerator** — target-agnostic export, compiled to whatever accelerator is on hand (else CPU/GPU parity as the honest fallback) + FPS/latency/memory benchmark. Target <1% AUROC/AU-PRO loss vs full precision. **Deploy-driven detector choice (a real result, not plumbing) ⚠️:** the memory-bank method (PatchCore) maps *awkwardly* to an accelerator — kNN/`cdist` against the bank isn't an accelerator primitive, so only the backbone offloads and the bank-distance stays host-side; a **conv reconstruction** model (feature-recon / VAE — int8-quantizable, no bank) is the clean fit. So the edge-deployable detector may be the reconstruction variant *despite* equal AU-PRO — which is why the bank-free latent score (`6v3`) matters. *(⚠️ = verify accelerator op-sets / int8 limits via the owed research pass before README.)* **Resolution** is a detector-hardening lever (`9rp`), measured on the current GPU (AU-PRO vs VRAM/FPS) — its cost curve *feeds* this stage as a **cost input**, it is not a blocker for it.
